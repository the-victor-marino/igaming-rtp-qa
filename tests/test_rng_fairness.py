...
def test_rng_passes_chi_square():
    ...
    assert is_fair(p_value), f"RNG failed fairness test (chi2={chi2:.3f}, p={p_value:.4f})"

def test_fairness_helper_interpretation():
    """is_fair must correctly interpret p-values against alpha."""
    assert is_fair(0.20) is True
    assert is_fair(0.01) is False
...
def test_biased_distribution_is_detected():
    ...
    assert not is_fair(p_value), "Audit failed to detect a biased distribution!"
